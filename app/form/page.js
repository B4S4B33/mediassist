...
export default function MedicalForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 503) {
        const errorMsg = 'Database is temporarily unavailable. Please try again in a moment.';
        setError(errorMsg);
        setLoading(false);
        return;
      }
      ...
      if (!conversationResponse.ok) {
        ...
        throw new Error(errorData.error || `Server error: ${conversationResponse.status}`);
      }
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {uploadedImages.map((image) => (
                      <div key={image.id} className="image-preview h-32">
                        <Image src={image.data} alt={image.name} width={128} height={128} />
                        <button
  ...
  // Redirect is handled by useEffect, but show loading during redirect
  if (!isSignedIn) {
    return null;
  }
